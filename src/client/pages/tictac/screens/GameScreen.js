...
export default class GameScreen extends PIXI.Container {
  ...
  async connect () {
    this.room = await colyseus.joinOrCreate('dm_tictactoe');
    //caob todo - variar nombre sala
  ...
  onJoin () {
    ...
    this.timeIcon = new PIXI.Sprite.fromImage('/src/client/images/clock-icon.png')
    ...
    this.timeRemaining.pivot.x = (this.timeRemaining.width / 2) + 72
    ...
    this.statusText = new PIXI.Text("Tu turno!", {
      font: "100px JennaSue",
      fill: '#fff',
      // fill: 0xfff,
      textAlign: 'center'
  ...
  nextTurn (playerId) {
    tweener.add(this.statusText).to({
      ...
      if (playerId == this.room.sessionId) {
        this.statusText.text = "tu turno!"

      } else {
        this.statusText.text = "turno del otro..."
      }
    ...
    this.timeRemaining.style.fill = '#f0f0f0';
